...
export default function EditProfileScreen() {
  ...
  // Merge profile + fallback to user
  const displayData =
    profile && Object.keys(profile).length > 0 ? profile : user;

  // Sync UI state whenever displayData changes
  useEffect(() => {
    if (displayData) {
      setName(displayData.complete_name || "");
      setUsername(displayData.username || "");
      setEmail(displayData.email || "");
      setBirthday(displayData.birthdate || "");
      setProfileImage(displayData.avatar_url || null);
    }
  }, [displayData]);
  ...
  const hasChanges =
    file ||
    name !== (displayData?.complete_name || "") ||
    username !== (displayData?.username || "") ||
    email !== (displayData?.email || "") ||
    birthday !== (displayData?.birthdate || "");
      ...
      <ScrollView>
        ...
        {message ? (
          ...
          >
            <Icon
              name={
                messageType === "success" ? "checkmark-circle" : "close-circle"
              }
              size={20}
