...
export default function TeacherMailScreen() {
  const { notifications, getNotification, markAsReadById, deleteNotificationById } = useNotificationStore();
  ...
  useFocusEffect(
      useCallback(() => {
        ...
        return () => {
          isActive = false; // cleanup if needed
        };
      }, [getNotification])
    );
  ...
  const markAsRead = async (id) => {
    try {
      ...
      setMessages((prev) =>
        prev.map((msg) => (msg.id === id ? { ...msg, is_read: 1 } : msg))
      );
    } catch (error) {
  ...
  const filteredMessages = messages
    .filter((msg) =>
      activeTab === 'All' ? true : activeTab === 'Unread' ? !msg.is_read : msg.is_read
    )
    .sort((a, b) => {
    ...
    <View style={styles.container}>
      ...
      {loading ? (
        ...
        <View style={styles.emptyContainer}>
          <Text style={styles.emptyText}>No message or Notification</Text>
        </View>
      ) : (
